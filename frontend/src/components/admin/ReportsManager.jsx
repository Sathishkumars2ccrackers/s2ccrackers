...
import { Download, TrendingUp, DollarSign, Package, ShoppingBag, FileSpreadsheet } from 'lucide-react';
    ...
    <div className="space-y-8">
      ...
      <div className="bg-festival-card border border-festival-border p-6 sm:p-8 rounded-3xl space-y-6">
        <div className="flex items-center gap-2.5 pb-4 border-b border-festival-border text-amber-400 font-bold text-base">
          ...
          <h3 className="text-white">Direct Excel (.xlsx) Data Export Hub</h3>
        </div>
            ...
            return (
              ...
              >
                ...
                <div className="flex items-center gap-2">
                  <a
                    href={analyticsService.getExportUrl(item.entity, 'xlsx')}
                    download
                    className="px-3 py-1.5 rounded-lg bg-emerald-950/80 border border-emerald-500/30 text-emerald-300 hover:text-white text-xs font-bold flex items-center gap-1 transition-colors"
                  >
                    <Download className="w-3 h-3" />
                    <span>.XLSX</span>
                  </a>
                  <a
                    href={analyticsService.getExportUrl(item.entity, 'csv')}
                    download
                    className="px-3 py-1.5 rounded-lg bg-festival-card border border-festival-border text-slate-300 hover:text-white text-xs font-bold flex items-center gap-1 transition-colors"
                  >
                    <Download className="w-3 h-3" />
                    <span>.CSV</span>
                  </a>
