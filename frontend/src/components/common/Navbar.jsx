...
const Navbar = () => {
  ...
  // Close mobile menu on page route changes
  useEffect(() => {
          ...
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 flex items-center justify-between gap-4">
            ...
            <div className="flex items-center gap-2 sm:gap-4">
              ...
              <motion.button
                onClick={openCart}
                whileTap={{ scale: 0.95 }}
                className="relative flex items-center gap-2.5 bg-gradient-to-r from-red-600 to-amber-600 hover:from-red-500 hover:to-amber-500 text-white px-4 py-2 rounded-full font-bold text-sm shadow-lg shadow-red-950/50 border border-amber-400/30 transition-all"
              >
                <div className="relative">
                  <ShoppingBag className="w-5 h-5" />
                  {totalItemsCount > 0 && (
                    <span className="absolute -top-2 -right-2 w-5 h-5 bg-white text-red-700 font-extrabold text-xs rounded-full flex items-center justify-center shadow-md animate-bounce">
                      {totalItemsCount}
                    </span>
                  )}
                </div>
        ...
        <AnimatePresence>
          {isMobileMenuOpen && (
            ...
            >
              ...
              {/* Navigation Links */}
              <ul className="space-y-2 pt-2 border-t border-festival-border/50 text-sm font-medium">
                {navLinks.map((link) => (
                  <li key={link.name}>
                    <Link
                      to={link.path}
